      ...
      <div style="display: flex; margin-bottom: 30px; margin-left: 30px;">
        <div>
          <el-button v-if="this.productState === '上架'" class="button" round
                     ...
                     @click="changeState('上架')">在卖({{ this.total }})
          </el-button>
          <el-button v-if="this.productState !== '上架'" class="button" round
                     style="border:0px;  background-color:rgb(242, 241, 246) ;" @click="changeState('上架')">在卖
          </el-button>
        </div>
        <div>
          <el-button v-if="this.productState !== '下架'" class="button" round
                     style="border:0px;  background-color:rgb(242, 241, 246) ;" @click="changeState('下架')">已下架
          </el-button>
          <el-button v-if="this.productState === '下架'" class="button" round
                     style="border:0px; font-weight: bold; background-color: #fff;color: #606266;"
                     @click="changeState('下架')">已下架({{ this.total }})
          </el-button>
        </div>
          ...
          <div style="width: 450px; display: flex;">
            ...
            <div style="width: 400px; height: 120px; padding: 10px 0 15px 15px;" @click="check(product.id)">
              ...
              <div style="font-size: 12px; margin-top: 22px; color:rgb(122,122,122)"
                ...
                <span v-if="product.update_time" style="margin-left: 1%;">更新 {{
                    product.update_time
                  }}</span>
              </div>
              <div style="font-size: 12px; margin-top: 22px; color:rgb(122,122,122)"
            ...
            <div style="width: 50px;">
              <div style="width: 50px; height: 39px;">
                <el-button type="text"
                           style="padding-right: 20px; float: right; color: rgb(25,156,96);"
                           v-if="productState === '上架'" @click="editProduct(product.id)">编辑
                </el-button>
              </div>
              <div style="width: 50px; height: 39px;">
                <el-popconfirm :title="productState === '上架' ? '确定下架吗？' : '确定上架吗？'"
                               @confirm="productState === '上架' ? takeDown(product.id) : takeUp(product.id)">
                  <el-button slot="reference" type="text"
                             style="padding-right: 20px; float: right; color: rgb(221, 173, 68);">
                    {{ productState === '上架' ? '下架' : '上架' }}
                  </el-button>
                </el-popconfirm>
              </div>
              <div style="width: 50px; height: 39px;">
                <el-popconfirm title="确定删除吗？" @confirm="deleteProduct(product.id)">
                  <el-button slot="reference" type="text"
                             style="padding-right: 20px; float: right; color: rgb(240, 37, 37);">删除
                  </el-button>
                </el-popconfirm>
              </div>
